...
- **Entity IDs as bare `string`** → branded. Salesforce record/org: `SalesforceId`/`OrgId` (`core/schemas/salesforceId.ts`) via `orgIdFromConnection`/`authFieldsFromConnection` (`core/schemas/authFields.ts`). `DefaultOrgInfoSchema.orgId`/`devHubOrgId`: `Schema.optional(OrgId)` like `cliId` — not `optionalWith` as Option. Else `Schema.UUID.pipe(Schema.brand(...))`.
- **Non-empty string `filterOrFail`** (`isNotUndefined && length > 0`) → `Schema.is(Schema.NonEmptyString)`. AuthFields org ids: `orgIdFromConnection` (`Option<OrgId>`) then `Option.match`. DefaultOrgInfo orgId: already `OrgId | undefined`.
