...
- W-23094919 sequencing check (informational, non-blocking):
  ...
  - This rule does NOT depend on W-23094919's code (disjoint rules; only shared file is `eslint.config.mjs`). Proceed regardless of W-23094919 status. If a human explicitly requires strict sequencing, HALT until W-23094919 is Closed/Completed AND on develop; otherwise continue.
...
- commit body (repo convention — all recent commits carry the trailer; config-only change has empty descriptive body, trailer only):
...
## Verification
- `npm run lint` clean (rule = error → 0 violations; shard across packages — `npx eslint .` OOMs on the full tree)
