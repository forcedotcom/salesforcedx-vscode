...
- services `activate()` returns the API at the **end** of the activation function only after `Effect.runPromise(activationEffect)` settles. `seedTelemetryIdentities` runs as a **blocking `yield*`** (not `Effect.fork*`) inside `activationEffect` **before** `updateTelemetryUserIds`, so by the time core's `getServicesApi` resolves, `defaultOrgRef.cliId` is populated.
...
Commit: `feat(services): seed telemetry identities on activation`
...
- new file `packages/salesforcedx-vscode-services/src/observability/seedTelemetryIdentities.ts` exporting `seedTelemetryIdentities` Effect, requires `ExtensionContextService`:
  ...
  - read `telemetryWebUserId` from globalState → `Option.fromNullable`:
    ...
    - `onSome`: leave as-is (`updateTelemetryUserIds` will mirror)
- `webUserId.ts:46-63 updateTelemetryUserIds`:
  - **drop** `TELEMETRY_GLOBAL_USER_ID` read + `userId` spread (`webUserId.ts:49,55`) — that path conflates cliId with SOQL `userId` slot. After Phase 1 the cliId slot is owned by `seedTelemetryIdentities`; `updateTelemetryUserIds` mirrors **only** `webUserId` from globalState into `defaultOrgRef`.
  - switch param `extensionContext: ExtensionContext` → use `ExtensionContextService` (consistent with `seedTelemetryIdentities`).
  - modernize ternary spread → `Option.fromNullable` + `Option.match`.
- `services/index.ts:220` ordering — confirmed atomic swap:
  1. drop `userId` writing from `updateTelemetryUserIds` (Phase 1 same commit)
  2. add blocking `yield* seedTelemetryIdentities` **before** `yield* updateTelemetryUserIds(context)` so `defaultOrgRef.cliId` is set before any consumer can read it
  3. activation continues only after both yields settle
