...
**Status (as of 2026-06-13):** All 9 OAS desktop Playwright specs are currently **skipped pending OAS LLM-service migration**. A4V v4.1.0 ("Agentforce Vibes") removed the `isEnabled` context key gating OAS command visibility and the `getLLMService` command / LLMService registration that OAS generation invokes. See `packages/salesforcedx-vscode-apex-oas/test/playwright/fixtures/desktopFixtures.ts` `beforeEach` hook for skip details.
