...
5. If working in packages with `test:web`/`test:desktop` scripts:
   - Package-level only (not in root): `salesforcedx-vscode-services` (web), `salesforcedx-vscode-org-browser`, `salesforcedx-vscode-metadata`, `salesforcedx-vscode-apex-testing`, `playwright-vscode-ext`
