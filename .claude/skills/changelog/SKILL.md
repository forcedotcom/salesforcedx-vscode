...
Background context for understanding the auto-generated commit; you don't typically interact with these details during polish.

### Generation

- Auto-generated by `Create Release Branch` GHA via conventional-changelog-action
- Commit: `chore: generated CHANGELOG for release/vXX.YY.ZZ`
- Human-edited after on release branch before publish (this skill)
- If nothing releasable (all `chore`/`ci` etc.), workflow skips, no entry added
...
- Type sections: `## Added` (from `feat`) and `## Fixed` (from `fix`). Ignored: `chore`, `style`, `refactor`, `test`, `build`, `ci`, `revert`
- Package sections: `#### <package-name>`, alphabetical within type
- Entries: `- <message> ([PR #N](url))`; format `https://github.com/forcedotcom/salesforcedx-vscode/pull/<num>`
- Multiple PRs: comma-separate `([PR #A](...), [PR #B](...), [ISSUE #C](...), [DISCUSSION #D](...))`
- Include `[ISSUE #N](https://github.com/forcedotcom/salesforcedx-vscode/issues/N)` or `[DISCUSSION #N](...)` when listed in PR's "What issues does this PR fix or reference?" section

### Package filtering (auto)

- `salesforcedx-vscode-core` touched → drop all other packages (except `docs`)
- Only packages starting with `salesforce` or `docs` count; `/images/` and `/test/` ignored

### Commit parsing (auto)

- Requires conventional `type(scope): message` + trailing `(#PR)`
- Strips `[W-XXXXXXXX]` GUS refs; uppercases first char
- Skips entries whose PR# exists (rerun-safe)

### Dedupe / merge rules (post-generation)

- Same PR under multiple packages → keep under most user-relevant, delete others (Rule 3)
- Same PR twice in section → collapse to one
- Empty `#### <package>` → remove during polish or keep if auto-generated; match style
- Patch rolling into next release → merge entries into later section, update header date; see `chore: merge ... changelogs`