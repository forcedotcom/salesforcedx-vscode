...
`getAuthInfoFields()` / `getFields()` → `authFieldsFrom` / `authFieldsFromConnection` / `orgIdFromConnection` (`core/schemas/authFields.ts`) — all `Option`. Fields: `optionalWith(..., { as: 'Option' })`. Invalid orgId → whole decode `None` (`decodeUnknownOption`). Fail-if-missing: `Option.match`. Do not read `.orgId` ad hoc.
