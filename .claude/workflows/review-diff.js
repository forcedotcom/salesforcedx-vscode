...
ADR-violation check. Read the accepted ADRs under ${wt}/docs/adr/ (repo-wide) plus any ${wt}/packages/*/docs/adr/ for packages the diff touches. If a diff hunk violates an accepted ADR's decision, emit a severity-graded finding. The finding's 'evidence' MUST embed a 'file:line' citation pointing at the specific ADR decision it violates (the ADR file + the line of the accepted-decision text), same as the file:line evidence rule above — so the citation is checkable in verification.`
...
- **dropped** — ONLY when you can affirmatively DISPROVE the finding via one of these (each is a proof, not a judgment call):
  ...
  - **ADR premise false**: the finding's claim cites an ADR the diff supposedly violates. READ the cited ADR file:line and its accepted-decision text. Drop ONLY if (a) the cited ADR file or decision does not exist (hallucinated citation), (b) the ADR is not \`Accepted\` — e.g. Proposed/Rejected/Deprecated/Superseded, so there is no binding decision to violate, or (c) you read both the ADR and the diff and the diff does not actually contradict the decision (the contradiction is factually absent). Inability to prove the diff COMPLIES is NOT a drop — that keeps the finding per "when in doubt, KEEP".
