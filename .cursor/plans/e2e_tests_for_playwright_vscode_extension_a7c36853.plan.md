...
todos:
  - id: setup-test-structure
    content: Create test directory structure (fixtures, specs, web) and base files
    status: pending
  - id: create-fixtures
    content: Create fixtures/index.ts and fixtures/desktopFixtures.ts for web/desktop test support
    status: pending
    dependencies:
      - setup-test-structure
  - id: create-web-server
    content: Create web/headlessServer.ts for web test server setup
    status: pending
    dependencies:
      - setup-test-structure
  - id: create-playwright-configs
    content: Create playwright.config.web.ts and playwright.config.desktop.ts
    status: pending
    dependencies:
      - setup-test-structure
  - id: test-command-palette
    content: Create commandPalette.headless.spec.ts testing F1, command execution, Windows fallback
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: test-file-operations
    content: Create fileOperations.headless.spec.ts testing file create, open, edit, save
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: test-settings
    content: Create settings.headless.spec.ts testing settings UI, search, textbox/checkbox modification
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: test-output-channel
    content: Create outputChannel.headless.spec.ts testing output panel, channel selection, text waiting
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: test-context-menu
    content: Create contextMenu.headless.spec.ts testing editor/explorer menus (skip Mac desktop)
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: test-helpers
    content: Create helpers.headless.spec.ts testing closeWelcomeTabs, waitForWorkspaceReady, etc
    status: pending
    dependencies:
      - create-fixtures
      - create-playwright-configs
  - id: update-package-json
    content: Add test scripts and wireit configs to package.json
    status: pending
    dependencies:
      - create-playwright-configs
  - id: create-github-workflow
    content: Create .github/workflows/playwrightVscodeExtE2E.yml for CI testing
    status: pending
    dependencies:
