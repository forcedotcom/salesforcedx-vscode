...
1.  Start with any existing failing (check local and lateste CI) from progress.txt\
...
7. push to github \
8. monitor the e2e run see @.cursor/commands/analyze-e2e.md for how to do it \
9. If failures, download logs/artifacts and update the progress.txt file \
10. If a commit made the result much worse, revert the commit and try somethinge else.\
11. make sure code aligns with with coding-playwright-tests.mdc rules and still passess. make sure the org-browser tests are still passing (they share the playwright ext) \
12. Append your progress to the progress.txt file. Do not make statements that are not supported by the code.\
13. Any test failures (locally or on CI) are caused by your change, there were no pre-existing failures.  DO NOT EXIT if there are test failures.  FIX THEM!\
