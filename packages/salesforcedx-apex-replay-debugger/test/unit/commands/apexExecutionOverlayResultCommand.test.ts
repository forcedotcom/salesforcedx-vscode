...
describe('ApexExecutionOverlayResult basic heapdump response parsing, no actionScript', () => {
  ...
  const responseFailure = '[{"message":"Session expired or invalid","errorCode":"INVALID_SESSION_ID"}]';
...
describe('ApexExecutionOverlayResult heapdump parsing with ActionScript SOQL results', () => {
  ...
  const expectedActionScript = "SELECT Id, Name, AccountNumber FROM Account WHERE Name like 'okToDelete%'";
  ...
  const expectedErrorActionScript = "SELECT Id, Name, AccountNumber FROM AccountOpps WHERE Name like 'okToDelete%'";
...
describe('ApexExecutionOverlayResult heapdump parsing with ActionScript SOQL results', () => {
  ...
  const expectedFailureRuntimeExceptionStackTrace = 'AnonymousBlock: line 1, column 1';
  const expectedFailureRuntimeExceptionMessage = 'System.MathException: Divide by 0';
  ...
  const expectedFailureCompilationErrorCompileProblem = 'Expression cannot be a statement.';
