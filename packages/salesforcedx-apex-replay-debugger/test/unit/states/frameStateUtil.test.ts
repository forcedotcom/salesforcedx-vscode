...
describe('Frame state utilities', () => {
  describe('Verify frame name generation', () => {
    ...
    it('EVENT_CODE_UNIT_STARTED only strips trigger prefix', () => {
      ...
      expect(FrameStateUtil.computeFrameName(fields)).to.equal('Nothing/Here.Matters');
    });
    ...
    it('EVENT_CONSTRUCTOR_ENTRY simple class', () => {
      ...
      expect(FrameStateUtil.computeFrameName(fields)).to.equal('SomeClassName.SomeClassName');
    });
    ...
    it('EVENT_CONSTRUCTOR_ENTRY inner class', () => {
      ...
      expect(FrameStateUtil.computeFrameName(fields)).to.equal('ClassName.InnerClassName.InnerClassName');
    });
    ...
    it('EVENT_VF_APEX_CALL_START parse method name from invoke', () => {
      ...
      expect(FrameStateUtil.computeFrameName(fields)).to.equal('ClassName.method()');
    });
  ...
  describe('Verify isExtraneousVFGetterOrSetterLogLine only matches the extraneous get/set log lines', () => {
    ...
    it('Bare Minimum Get Match returns true', () => {
      ...
      expect(FrameStateUtil.isExtraneousVFGetterOrSetterLogLine(logLinePiece)).to.equal(true);
    });
    ...
    it('Malformed set returns false', () => {
      const logLinePiece = 'set(ThisPartOfTheStringShouldNotMatter)';
      expect(FrameStateUtil.isExtraneousVFGetterOrSetterLogLine(logLinePiece)).to.equal(false);
    });
