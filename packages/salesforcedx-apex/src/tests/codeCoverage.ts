...
export class CodeCoverage {
  ...
  public async getPerClassCodeCoverage(apexTestClassSet: Set<string>): Promise<Map<string, PerClassCoverage[]>> {
    HeapMonitor.getInstance().checkHeapSize('codeCoverage.getPerClassCodeCoverage');
    try {
      ...
      perClassCodeCovResults.forEach(chunk => {
        chunk.records.forEach(item => {
          ...
          const existing = perClassCoverageMap.get(key);
          if (existing) {
            existing.push(value);
          } else {
  ...
  public async getAggregateCodeCoverage(apexClassIdSet: Set<string>): Promise<{
    ...
    HeapMonitor.getInstance().checkHeapSize('codeCoverage.getAggregateCodeCoverage');
    try {
  ...
  private async queryAggregateCodeCov(apexClassIdSet: Set<string>): Promise<ApexCodeCoverageAggregate[]> {
    ...
    if (apexClassIdSet.size === 0) {
      ...
      const result = await queryAll<ApexCodeCoverageAggregateRecord>(this.connection, query, true);
      return [result];
    }
