...
describe('Date Utils', () => {
  ...
  it('should format a date to locale by default', () => {
    ...
    expect(dateUtil.formatStartTime(testStartTime)).toEqual(expectedFormat);
  });

  it('should format a date to locale by param', () => {
    const expectedFormat = `${testStartTimeDate.toDateString()} ${testStartTimeDate.toLocaleTimeString()}`;
    expect(dateUtil.formatStartTime(testStartTime, 'locale')).toEqual(expectedFormat);
  });

  it('should format a date to ISO', () => {
    expect(dateUtil.formatStartTime(testStartTime, 'ISO')).toEqual(testStartTimeDate.toISOString());
  });
});
