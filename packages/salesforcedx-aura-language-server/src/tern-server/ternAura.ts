...
const readFile = async (filename: string): Promise<string> => {
  ...
  try {
    const content = await server.fileSystemProvider.getFileContent(`file://${normalized}`);
    return content ?? '';
  } catch {
      ...
      (err: any, result: any) => {
        ...
        if (shouldFilter) {
          result.completions = result.completions.filter((completion, _index, _array) => {
            const accepted =
              (completion.doc?.includes('@platform') && completion.origin === 'Aura') ?? completion.origin !== 'Aura';
