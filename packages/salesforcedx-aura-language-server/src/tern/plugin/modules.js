(function (mod) {
  ...
  Modules.prototype = signal.mixin({
    ...
    maybeOverride: function (name) {
      if (!this.options.modules || !this.options.modules.hasOwnProperty(name)) return false;
      if (this.modules[name]) return this.modules[name];

      var override = this.options.modules[name];
      if (typeof override == 'string' && override.charAt(0) == '=') return infer.def.parsePath(override.slice(1));
    ...
    resolveModule: function (name, parentFile) {
      ...
      var resolved,
        relative = isRelative(name);
      for (var i = 0; !resolved && i < this.resolvers.length; i++) resolved = this.resolvers[i](name, parentFile);
      ...
      if (/\.js$|(?:^\/)[^\.]+$/.test(resolved)) this.server.addFile(resolved, null, parentFile);
    ...
    isModName: function (node, pos) {
      return this.findIn(this.modNameTests, node, pos);
    },
    isImport: function (node, pos) {
      return this.findIn(this.importTests, node, pos);
    },
    ...
    completeModuleName: function (completions, query, word) {
      function fromObj(obj, useVal) {
        for (var name in obj)
          if (filter(word, name, query)) tern.addCompletion(query, completions, name, useVal && obj[name]);
      }
    ...
    getModType: function (node) {
      var modName = this.isModName(node),
        imp,
        prop;
  ...
  function filter(word, string, query) {
    return (
      query.filter === false || !word || (query.caseInsensitive ? string.toLowerCase() : string).indexOf(word) == 0
    );
  }
  ...
  function postLoadDef(data) {
    ...
    if (known) for (var name in known.props) me.knownModules[name.replace(/`/g, '.')] = known.props[name];
  }
  ...
  function findCompletions(file, query) {
    ...
    var wordEnd = tern.resolvePos(file, query.end),
      types = me.completableTypes;
    var expr = infer.findExpressionAround(file.ast, null, wordEnd, file.scope, function (type) {
      return type in types;
    });
    if (!expr) return null;

    if (me.isModName(expr.node, wordEnd) != null) return findModuleCompletions(me, file, query, expr.node, wordEnd);

    var imp = me.isImport(expr.node, wordEnd);
    if (imp && imp.name && imp.prop != null) return findImportCompletions(me, file, query, expr.node, imp, wordEnd);
  }
  ...
  tern.registerPlugin('modules', function (server, options) {
    // Ensure server.mod exists before setting server.mod.modules
    server.mod ??= {};
  ...
  tern.defineQueryType('exports', {
    ...
    run: function (server, query, file) {
      ...
      var mod = server.mod.modules,
        known = mod && mod.modules[file.name];
      if (!known) return {};
      var resp = describe(known);
      var type = known.getType(false);
      if (type instanceof infer.Obj) {
        var props = (resp.props = {});
        for (var prop in type.props) props[prop] = describe(type.props[prop]);
      }
      return resp;
    }
  });
