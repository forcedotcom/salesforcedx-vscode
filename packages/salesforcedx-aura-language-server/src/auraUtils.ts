...
const hasQuotes = (str: string): boolean =>
  (str.at(0) === '"' && str.at(-1) === '"') || (str.at(0) === "'" && str.at(-1) === "'");

const getTagNameRange = (
 ...
 */
export const getAuraBindingTemplateDeclaration = (
  document: TextDocument,
  position: Position,
  htmlDocument: HTMLDocument
): Location | null => {
 ...
 */
export const getAuraBindingValue = (
  document: TextDocument,
  position: Position,
  htmlDocument: HTMLDocument
): string | null => {
 ...
 */
const extractPropertyNameFromExpression = (
  content: string,
