...
emptyWorkspaceDesktopTest('SFDX Create Project commands: no folder open (workspace closed)', async ({ page }) => {
  emptyWorkspaceDesktopTest.setTimeout(120_000);
  await prepareNoFolderOpenForPaletteTests(page);
  await assertCreateProjectCommands(page);
});

emptyWorkspaceDesktopTest('SFDX Create Project commands: folder open without sfdx-project.json', async ({ page }) => {
  emptyWorkspaceDesktopTest.setTimeout(120_000);
