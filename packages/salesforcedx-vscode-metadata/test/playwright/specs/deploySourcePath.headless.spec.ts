...
test.describe('Deploy Source Path', () => {
  test('deploys via all entry points', async ({ page }) => {
    ...
    await test.step('2. Editor context menu', async () => {
      ...
      await saveScreenshot(page, 'step2.before-context-menu.png');
      ...
      await saveScreenshot(page, 'step2.after-context-menu-command.png');
    ...
    await test.step('3. Explorer context menu (file)', async () => {
      ...
      await saveScreenshot(
        page,
        `step3.before-edit-counts-${countsBeforeEdit.local}-${countsBeforeEdit.remote}-${countsBeforeEdit.conflicts}.png`
      );
      ...
      await saveScreenshot(page, 'step3.before-explorer-context-menu.png');
    ...
    await test.step('4. Explorer context menu (directory)', async () => {
      // Close any open editors to ensure clean state
      await executeCommandWithCommandPalette(page, 'View: Close All Editors');
      await saveScreenshot(page, 'step4.after-close-editors.png');

      // Ensure status bar is ready and file is synced (local=0) after step 3 deploy
      await statusBarPage.waitForVisible(10_000);
      await statusBarPage.waitForCounts({ local: 0 }, 30_000);
      await saveScreenshot(page, 'step4.after-sync-confirmed.png');

      // Edit class again to create new local change
      await openFileByName(page, `${className}.cls`);
      await saveScreenshot(page, 'step4.after-open-file.png');
      // Ensure the editor is focused before editing
      const apexEditor = page.locator(`[data-uri*="${className}.cls"]`).first();
      await apexEditor.waitFor({ state: 'visible', timeout: 10_000 });
      await apexEditor.click();
      await editOpenFile(page, 'Explorer directory context menu test');
      await saveScreenshot(page, 'step4.after-edit.png');
      const countsAfterEdit = await statusBarPage.getCounts();
      await saveScreenshot(
        page,
        `step4.after-edit-counts-${countsAfterEdit.local}-${countsAfterEdit.remote}-${countsAfterEdit.conflicts}.png`
      );
      await statusBarPage.waitForCounts({ local: 1 }, 60_000);
      await saveScreenshot(page, 'step4.after-local-count-1.png');
      ...
      await saveScreenshot(page, 'step4.after-explorer-context-menu-command.png');

      // Check for deploy-related error notifications before waiting for deploying notification
      const allNotifications = page.locator('.monaco-workbench .notification-list-item');
      await saveScreenshot(page, 'step4.checking-notifications.png');
      const deployErrorNotification = allNotifications
        .filter({ hasText: /Failed to deploy|ENOENT|deploy.*failed/i })
        .first();
      const hasDeployError = await deployErrorNotification.isVisible({ timeout: 2000 }).catch(() => false);
      if (hasDeployError) {
        await saveScreenshot(page, 'step4.deploy-error.png');
        const errorText = await deployErrorNotification.textContent();
        throw new Error(`Deploy failed with error notification: ${errorText}`);
      }
      ...
      // Verify deploy completes
      const deployingNotification = await waitForDeployProgressNotificationToAppear(page, 30_000);
      await saveScreenshot(page, 'step4.deploy-notification-appeared.png');
      await expect(deployingNotification).not.toBeVisible({ timeout: 240_000 });
      await statusBarPage.waitForCounts({ local: 0 }, 60_000);
      await saveScreenshot(page, 'step4.deploy-complete.png');
    });
