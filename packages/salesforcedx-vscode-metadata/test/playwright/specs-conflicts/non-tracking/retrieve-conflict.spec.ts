...
test.describe('Retrieve Conflict Detection (Non-Source Tracking)', () => {
  test('detects conflict, views diff, then overrides and retrieves', async ({ page, helperProject }) => {
    ...
    await test.step('1. Create and deploy baseline', async () => {
      ...
      await saveScreenshot(page, 'non-tracking-retrieve-1-created.png');

      await deployCurrentSourceToOrg(page);
      await saveScreenshot(page, 'non-tracking-retrieve-2-deployed.png');
    });
    ...
    await test.step('3. Modify locally (different from remote)', async () => {
      ...
      await saveScreenshot(page, 'non-tracking-retrieve-3-local-modified.png');
    });
    ...
    await test.step('5. View Conflicts - tree shows file, clicking opens diff editor', async () => {
      ...
      await saveScreenshot(page, 'non-tracking-retrieve-5-view-conflicts-clicked.png');

      await tree.waitForItem(`${className}.cls`);
      await saveScreenshot(page, 'non-tracking-retrieve-6-tree-item-visible.png');

      await tree.clickItem(`${className}.cls`);
      await diff.waitForTab(className);
      await saveScreenshot(page, 'non-tracking-retrieve-7-diff-editor.png');
    });
    ...
    await test.step('6. Override conflicts and retrieve successfully', async () => {
      ...
      await saveScreenshot(page, 'non-tracking-retrieve-9-retrieved.png');
    });

    await test.step('7. Validate conflict cleared after override retrieve', async () => {
      await tree.waitForItemGone(`${className}.cls`);
      await saveScreenshot(page, 'non-tracking-retrieve-10-conflict-cleared.png');
    });
  });
});
