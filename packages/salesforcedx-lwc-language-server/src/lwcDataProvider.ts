...
export class LWCDataProvider implements IHTMLDataProvider {
  ...
  private _standardTags: ITagData[] = (transformedLwcStandard as { tags: ITagData[] }).tags.map(transformTag);
  // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
  private _globalAttributes: IAttributeData[] =
    // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
    (transformedLwcStandard as { globalAttributes: IAttributeData[] }).globalAttributes.map(transformAttribute);
