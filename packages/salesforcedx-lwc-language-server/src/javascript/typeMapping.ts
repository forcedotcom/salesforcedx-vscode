 ...
 * value is `undefined`.
 */
const stripKeysWithUndefinedVals = <T extends Record<string, any>>(obj: T): T =>
...
const dataPropertyToPropValue = (decoratorType: DecoratorValType | undefined, extDataProp?: DataProperty): ClassMemberPropertyValue | undefined => {
    if (!extDataProp) {
        return;
    }
 ...
 */
const externalToInternalPropValue = (
    decoratorType: DecoratorValType | undefined,
    initialValue: Value,
    isWireParam = false,
...
const getMemberProperty = (propertyObj: ClassProperty): InternalClassMember | null => {
    ...
    // Handle properties with no decorators (they default to undefined)
...
const getMembers = (classObj: Class): InternalClassMember[] => {
    const properties: InternalClassMember[] = classObj.properties.map(getMemberProperty).filter((member): member is InternalClassMember => member !== null);
    const methods: InternalClassMember[] = classObj.methods.map(getMemberMethod).filter((member): member is InternalClassMember => member !== null);
...
const getWireParams = (decorator: WireDecorator) => {
    ...
    if (decorator.adapterConfig) {
        ...
        staticObj = Object.fromEntries(
            Object.entries(decorator.adapterConfig.static)
                .map(([key, staticParam]) => [key, externalToInternalPropValue('wire', staticParam.value, true)])
...
const getDecoratedProperties = (
    ...
    for (const prop of properties) {
        ...
        propLocs.set(prop.name, getPropLoc(prop) ?? 0);
...
const getDecorators = (classObj: Class): InternalDecorator[] => {
    ...
    const wire: InternalWireDecorator | null =
        wiredMethods.length > 0 || wiredProperties.length > 0
            ...
            : null;
    const track: InternalTrackDecorator | null =
        trackedProperties.length > 0
            ...
            : null;
    const api: InternalApiDecorator | null =
...
const getExports = (lwcExports: LwcExport[]): InternalModuleExports[] =>
    lwcExports.flatMap((lwcExport): InternalModuleExports | InternalModuleExports[] => {
        if (lwcExport.namedExports) {
            return lwcExport.namedExports.map(
                (namedExport): InternalModuleExports =>
...
export const mapLwcMetadataToInternal = (lwcMeta: ScriptFile): InternalMetadata => {
    ...
    const decorators = getDecorators(mainClassObj);
    const classMembers = getMembers(mainClassObj);
    const exports = getExports(lwcMeta.exports);
