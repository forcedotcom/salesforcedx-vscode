...
const dataPropertyToPropValue = (
  decoratorType: DecoratorValType | undefined,
  extDataProp?: DataProperty
    ...
    case 'Object':
      ...
      return {
        ...
        value: Object.fromEntries(
          Object.entries(initialValue.value).map(([key, val]) => [key, externalToInternalPropValue(decoratorType, val)])
        )
      };
...
const getWireParams = (decorator: WireDecorator) => {
  ...
  if (decorator.adapterConfig) {
    ...
    params = Object.fromEntries(
      Object.entries(decorator.adapterConfig.reactive).map(([key, { classProperty }]) => [key, classProperty])
    );
  }
  return {
    staticObj,
