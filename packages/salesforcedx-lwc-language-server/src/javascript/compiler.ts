...
export const getClassMembers = (metadata: Metadata, memberType: string, memberDecorator?: string): ClassMember[] =>
  (metadata.classMembers ?? []).filter(
    member => member.type === memberType && (!memberDecorator || member.decorator === memberDecorator)
  );
...
export const toVSCodeRange = (babelRange: SourceLocation): Range =>
  ...
  Range.create(
    Position.create(babelRange.start.line - 1, babelRange.start.column),
    Position.create(babelRange.end.line - 1, babelRange.end.column)
  );

export const extractAttributes = (
  metadata: Metadata,
  uri: string
): { privateAttributes: AttributeInfo[]; publicAttributes: AttributeInfo[] } => {
