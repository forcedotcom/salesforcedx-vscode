...
// e2e-COVERED: `sf.org.login.access.token` (migrated to an Effect command) activation + prompt-appears + Esc cancel.
...
// input box, and Esc maps UserCancellationError -> silent cancel (no error toast). The success/Bad_OAuth_Token
// paths are covered by jest (orgLoginAccessToken.test.ts + connectionService.loginWithAccessToken.test.ts).
