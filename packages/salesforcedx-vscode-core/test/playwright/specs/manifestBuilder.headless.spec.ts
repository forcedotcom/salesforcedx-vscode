...
(isMacDesktop() ? test.skip.bind(test) : test)('Manifest Builder: generate manifest, deploy and retrieve via manifest', async ({ page }) => {
  ...
  await test.step('deploy source in manifest', async () => {
    ...
    await waitForOutputChannelText(page, { expectedText: 'Ended SFDX: Deploy Source in Manifest to Org', timeout: COMMAND_TIMEOUT });
  ...
  await test.step('retrieve source in manifest', async () => {
    ...
    await waitForOutputChannelText(page, { expectedText: 'Ended SFDX: Retrieve Source in Manifest from Org', timeout: COMMAND_TIMEOUT });
