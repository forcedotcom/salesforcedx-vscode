...
test('Push and Pull: push, pull, and view changes', async ({ page }) => {
  ...
  await test.step('view all changes (empty)', async () => {
    ...
    await executeCommandWithCommandPalette(page, 'SFDX: View All Changes (Local and in Default Org)');
    await waitForOutputChannelText(page, { expectedText: 'No local or remote changes found', timeout: COMMAND_TIMEOUT });
  ...
  await test.step('push source to org', async () => {
    ...
    await waitForOutputChannelText(page, { expectedText: 'Ended SFDX: Push Source to Default Org', timeout: COMMAND_TIMEOUT });
  ...
  await test.step('pull source from org', async () => {
    ...
    await waitForOutputChannelText(page, { expectedText: 'Ended SFDX: Pull Source from Default Org', timeout: COMMAND_TIMEOUT });
