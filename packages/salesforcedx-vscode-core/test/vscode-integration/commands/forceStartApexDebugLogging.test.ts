...
describe('Force Start Apex Debug Logging', () => {
  ...
  it('Should build the traceflag query command for logging', async () => {
    ...
    expect(updateTraceFlagCmd.toCommand()).to.equal(
      `sfdx data:query --query SELECT id, logtype, startdate, expirationdate, debuglevelid, debuglevel.apexcode, debuglevel.visualforce FROM TraceFlag WHERE logtype='DEVELOPER_LOG' AND TracedEntityId='005x00000000123' --use-tooling-api --json`
    );
  });
  ...
  it('Should build the traceflag update command for logging', async () => {
    ...
    expect(updateTraceFlagCmd.toCommand()).to.equal(
      `sfdx data:update:record --sobject TraceFlag --record-id ${fakeTraceFlagId} --values StartDate='' ExpirationDate='${endDate.toUTCString()}' --use-tooling-api --json`
    );
  });
  ...
  it('Should build the debuglevel update command for logging', async () => {
    ...
    expect(updateDebugLevelCmd.toCommand()).to.equal(
      `sfdx data:update:record --sobject DebugLevel --record-id ${fakeDebugLevelId} --values ApexCode=${APEX_CODE_DEBUG_LEVEL} Visualforce=${VISUALFORCE_DEBUG_LEVEL} --use-tooling-api --json`
    );
  });
  ...
  it('Should build the traceflag create command for logging', async () => {
    ...
    expect(createTraceFlagCmd.toCommand()).to.equal(
      `sfdx data:create:record --sobject TraceFlag --values tracedentityid='testUserId' logtype=developer_log debuglevelid=${fakeDebugLevelId} StartDate='' ExpirationDate='${endDate.toUTCString()} --use-tooling-api --json`
    );
  });
  ...
  it('Should build the debuglevel create command for logging', async () => {
    ...
    expect(createDebugLevelCmd.toCommand()).to.equal(
      `sfdx data:create:record --sobject DebugLevel --values developername=${createDebugLevelExecutor.developerName} MasterLabel=${createDebugLevelExecutor.developerName} apexcode=${APEX_CODE_DEBUG_LEVEL} visualforce=${VISUALFORCE_DEBUG_LEVEL} --use-tooling-api --json`
    );
  });
  ...
  it('Should build the user id query command', async () => {
    ...
    expect(forceQueryUserCmd.toCommand()).to.equal(
      `sfdx data:query --query SELECT id FROM User WHERE username='${testUser}' --json`
    );
  });
});
