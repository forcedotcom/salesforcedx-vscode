...
describe('Force Start Apex Debug Logging', () => {
  ...
  it('Should build the debuglevel create command for logging', async () => {
    ...
    expect(createDebugLevelCmd.toCommand()).to.equal(
      `sfdx force:data:record:create --sobjecttype DebugLevel --values developername=${
        createDebugLevelExecutor.developerName
      } MasterLabel=${
        createDebugLevelExecutor.developerName
      } apexcode=${APEX_CODE_DEBUG_LEVEL} visualforce=${VISUALFORCE_DEBUG_LEVEL} --usetoolingapi --json --loglevel fatal`
    );
  });
