...
describe('Status', () => {
  it('Should build the source command no flag', async () => {
    ...
    expect(displayCommand.description).to.equal(nls.localize('org_display_default_text'));
  });
  it('Should build the source command with target-org flag', async () => {
    ...
    expect(displayCommand.toCommand()).to.equal('sf org:display --target-org test');
    expect(displayCommand.description).to.equal(nls.localize('org_display_username_text'));
  });
});
