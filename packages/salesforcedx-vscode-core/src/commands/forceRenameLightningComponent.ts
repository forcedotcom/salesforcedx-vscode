...
export function getParentDirectoryOfTestFolder(sourceFsPath: string, pathSeparator: string): string {
  const directories = sourceFsPath.split(pathSeparator);
  directories.splice(directories.length - 1);
  return directories.join(pathSeparator);
}
