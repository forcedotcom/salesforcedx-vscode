...
public class ${className} {
    ...
    await test.step('discover and open the org-only class virtual doc', async () => {
      ...
      // navigating to the `sf-org-data:` virtual doc, so close all editors first.
      ...
      // "go to test" navigation, which opens the method's `sf-org-data:` virtual doc (the one place
      ...
      await expect(page.locator(`${EDITOR_WITH_URI}[data-uri^="sf-org-data:"]`).first()).toBeVisible({
        timeout: 60_000
      });
