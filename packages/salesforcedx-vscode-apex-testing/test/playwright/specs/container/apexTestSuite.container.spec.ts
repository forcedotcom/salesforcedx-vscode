...
/*
 * Container port of apexTestSuite.headless.spec.ts. The web twin never surfaces Apex
 * test suites because salesforcedx-vscode-apex has no browser bundle. The Code Builder image runs
 * the DESKTOP build in a Node host, so the full Create / Edit / Run Apex Test Suite palette flow and
 * the Test Explorer suite tree work end-to-end against the boot org (one tracking scratch org authed
 * as default target-org). It reuses the seeded PagedResultTest + ExampleClassTest classes and only
 * creates a uniquely-named suite so the shared, persistent workbench/org never collides.
 */
...
const selectSuiteInQuickPick = async (
  ...
  // highlights the row on slower runners without accepting it, leaving the picker open so the
  // command never runs and the output channel stays empty until TEST_RUN_TIMEOUT. Only press Enter
  // if the picker is still on the suite prompt (input still holds the typed name); if the click
  // already accepted and advanced to a follow-up prompt, do not fire a stray Enter.
...
/** Select a test class in a quick pick (type to filter, toggle checkbox, then Enter). */
...
test('Apex Test Suite: create, verify creation, edit tests, run suite', async ({ page }) => {
  ...
  await test.step('create Apex Test Suite with first class', async () => {
    ...
    await createApexTestSuiteViaPalette(page, testSuiteName, TEST_CLASS_1);
  ...
  await test.step('edit suite to add second test class', async () => {
    ...
    await selectTestClassInQuickPick(page, TEST_CLASS_2);
  ...
  await test.step('run Apex Test Suite', async () => {
    ...
    // still healthy. Gate on the run's progress toast instead: wait for it to appear (so the
    // gone-check below can't false-pass before the run started), then wait for it to clear.
  ...
  await test.step('verify test suite execution output', async () => {
    ...
    // Backstop timeout in case the progress toast raced ahead of the gate above; normally the run
    // has already completed so this resolves immediately against the populated channel.
    ...
    await waitForOutputChannelText(page, { expectedText: TEST_CLASS_1, timeout: 60_000 });
    await waitForOutputChannelText(page, { expectedText: TEST_CLASS_2, timeout: 60_000 });
  ...
  await test.step('verify removed class absent from test explorer tree', async () => {
    ...
    // Suite children are lazy-loaded from the org after expand — poll until TEST_CLASS_1 appears
    // AND TEST_CLASS_2 is gone (both must be true; checking only presence lets the poll pass while
    // TEST_CLASS_2 is still present due to eventual-consistency lag in the org).
    await expect
      .poll(
        async () => {
          const children = await getSuiteChildrenText(panel, testSuiteName);
          return {
            hasClass1: children.some(t => t.includes(TEST_CLASS_1)),
            hasClass2: children.some(t => t.includes(TEST_CLASS_2))
          };
        },
        { timeout: 30_000 }
      )
    ...
    expect(suiteChildren.some(t => t.includes(TEST_CLASS_2))).toBe(false);
  ...
  await test.step('re-run suite and verify removed class does not execute', async () => {
    ...
    await expect(outputPanel).not.toContainText(TEST_CLASS_2, { timeout: 30_000 });
