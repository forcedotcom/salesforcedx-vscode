...
/*
 * Container parity for Apex unit-test class scaffolding. The web twin
 * (apexTestClassCreate.headless.spec.ts) proves the command palette flow against a plain Page; this
 * proves the ApexUnitTest template actually lands a `.cls` file in the workspace from inside the Code
...
test('Create Apex Unit Test Class (Code Builder): creates a test class via command palette', async ({ page }) => {
  ...
  await test.step('workbench ready', async () => {
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.01-ready.png');
  });
  ...
  await test.step('run Create Apex Class command', async () => {
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.02-command-triggered.png');
  });
  ...
  await test.step('select ApexUnitTest template in QuickPick', async () => {
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.03-template-selected.png');
  });
  ...
  await test.step('enter class name in InputBox', async () => {
    ...
    await page.keyboard.press('Enter');
    await saveScreenshot(page, 'apexTestClassCreate.container.04-class-name-entered.png');
  });

  await test.step('select output directory in QuickPick', async () => {
    await waitForQuickInputFirstOption(page);
    await saveScreenshot(page, 'apexTestClassCreate.container.05-directory-prompt-visible.png');
    await page.keyboard.press('Enter');
    await page.locator(QUICK_INPUT_WIDGET).waitFor({ state: 'hidden', timeout: 10_000 });
    await saveScreenshot(page, 'apexTestClassCreate.container.06-after-accept-directory.png');
  });
  ...
  await test.step('verify editor opens with new class file', async () => {
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.07-editor-opened.png');
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.08-file-in-explorer.png');
    ...
    await saveScreenshot(page, 'apexTestClassCreate.container.09-class-content-verified.png');
  });
