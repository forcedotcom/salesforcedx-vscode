...
/*
 * Container parity for Apex debug-log retrieval. The web twin
 * (logRetrieval.headless.spec.ts) exercises the flow against a plain Page; this proves the full
 * round-trip against the container's boot-authed org — turn on a trace flag, run anonymous Apex to
 * produce an ApexLog record, then pull it back via `logGet` and open it in the editor. Web mode
 * cannot cover the org-side log generation and retrieval.
 */
...
test('Log retrieval (Code Builder): get logs, open folder', async ({ page }) => {
  ...
  await test.step('workbench ready', async () => {
    ...
    await saveScreenshot(page, 'logRetrieval.container.01-ready.png');
  });
  ...
  await test.step('generate log via execute anonymous apex', async () => {
    ...
    await saveScreenshot(page, 'logRetrieval.container.02-executed.png');
  });
  ...
  await test.step('get apex debug log and verify quick pick shows entries', async () => {
    ...
    await saveScreenshot(page, 'logRetrieval.container.03-quick-pick.png');
  });
  ...
  await test.step('verify log file opens in editor', async () => {
    ...
    await saveScreenshot(page, 'logRetrieval.container.04-log-opened.png');
  });
  ...
  await test.step('open logs folder and verify explorer', async () => {
    ...
    await saveScreenshot(page, 'logRetrieval.container.05-explorer.png');
  });
