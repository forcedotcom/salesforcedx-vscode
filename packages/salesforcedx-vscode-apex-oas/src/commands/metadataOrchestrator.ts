...
export const validateMetadata = async (
  ...
  const isEligibleResponses = await validateEligibility(sourceUri, isMethodSelected);
...
const eligibilityDelegate = async (
...
export const gatherContext = async (sourceUri: URI | URI[]): Promise<ApexClassOASGatherContextResponse | undefined> => {
  let response: ApexClassOASGatherContextResponse | undefined;
  const languageClientManager = await getApexLanguageClient();
  const languageClient = languageClientManager?.getClientInstance();
  if (languageClient) {
    try {
      response = await languageClient?.gatherOpenAPIContext(sourceUri ?? vscode.window.activeTextEditor?.document.uri);
...
const validateEligibility = async (
  ...
  const responses = await eligibilityDelegate({ payload: requests });
  return responses;
};
