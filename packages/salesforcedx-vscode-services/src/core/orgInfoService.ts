...
export class OrgInfoService extends Effect.Service<OrgInfoService>()('OrgInfoService', {
  ...
  effect: Effect.gen(function* () {
    ...
    /**
     * Resolve `username` (or the project default) to an org, get its `Connection` via
     * `ConnectionService.getConnection(username)`, and derive `OrgInfo`. Connection creation/query
     * failures (and web unsupported) degrade to an error-status table rather than crashing org-display.
     */
    const getOrgInfoForUsername = Effect.fn('OrgInfoService.getOrgInfoForUsername')(function* (username?: string) {
      ...
      return yield* connectionService.getConnection(resolvedUsername).pipe(
