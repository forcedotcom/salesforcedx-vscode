...
import type { OrgMetadataShadowStore } from './orgMetadataShadowStore';
import type { ComponentSetService } from '../core/componentSetService';
import type { MetadataRetrieveService } from '../core/metadataRetrieveService';
...
type OrgCatalogRemoteRetrieveOptions = {
  readonly componentSetService: InstanceType<typeof ComponentSetService>;
  readonly fsService: InstanceType<typeof FsService>;
  readonly metadataRetrieveService: InstanceType<typeof MetadataRetrieveService>;
  ...
  readonly getTypeSuffix: (xmlName: string) => string | undefined;
};

export const makeOrgCatalogRemoteRetrieve = ({
  componentSetService,
  fsService,
  ...
  documentUri,
  getTypeSuffix
}: OrgCatalogRemoteRetrieveOptions) => {
  ...
  const sourceBasenames = (orgId: string, reference: OrgMetadataComponentReference) => {
    ...
    const suffix = getTypeSuffix(reference.xmlName);
    return new Set([
      logicalBasename,
      `${logicalBasename}-meta.xml`,
      `${leafName}.${suffix}`,
      `${leafName}.${suffix}-meta.xml`
    ]);
  };
