...
import type { makeOrgCatalogRemoteRetrieve } from './orgCatalogRemoteRetrieve';
...
import type { OrgMetadataShadowArtifact, OrgMetadataShadowStore } from './orgMetadataShadowStore';
import type { ConnectionService } from '../core/connectionService';
...
type OrgCatalogRemoteSourceOptions = {
  readonly connectionService: InstanceType<typeof ConnectionService>;
  readonly fsService: InstanceType<typeof FsService>;
  readonly inventories: ReturnType<typeof makeOrgCatalogInventory>;
  readonly remoteRetrieve: ReturnType<typeof makeOrgCatalogRemoteRetrieve>;
  readonly shadowStore: InstanceType<typeof OrgMetadataShadowStore>;
...
export const makeOrgCatalogRemoteSource = Effect.fn('OrgCatalogRemoteSource.make')(function* ({
  connectionService,
  fsService,
  inventories,
  remoteRetrieve,
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.forEach(uniqueReferences, reference => {
          ...
          return artifact
            ...
            : Effect.die(new Error(`No shadow artifact was produced for ${reference.xmlName} '${reference.fullName}'`));
        });
      })
    );
  });
