...
import type { TransmogrifierService } from '../core/transmogrifierService';
import * as Effect from 'effect/Effect';
import * as Stream from 'effect/Stream';

type SObjectState = {
  readonly ensureHydrated: (orgId: string) => Effect.Effect<void>;
  readonly getSObjectDescription: (orgId: string, apiName: string) => Effect.Effect<OrgSObjectDescription | undefined>;
  readonly getSObjectDescriptions: (
    orgId: string,
    apiNames: readonly string[]
  ) => Effect.Effect<ReadonlyMap<string, OrgSObjectDescription>>;
  readonly getSObjectList: (orgId: string) => Effect.Effect<readonly OrgSObjectSummary[] | undefined>;
  readonly invalidateSObjects: (orgId: string, apiNames?: ReadonlySet<string>) => Effect.Effect<void>;
  readonly persistOrg: (orgId: string) => Effect.Effect<void>;
  readonly removeSObjectDescriptions: (orgId: string, apiNames?: ReadonlySet<string>) => Effect.Effect<void>;
  readonly setSObjectDescription: (orgId: string, description: OrgSObjectDescription) => Effect.Effect<void>;
  readonly setSObjectList: (orgId: string, observations: readonly OrgSObjectSummary[]) => Effect.Effect<void>;
};

type OrgCatalogSObjectsOptions = {
  readonly state: SObjectState;
  readonly metadataDescribeService: InstanceType<typeof MetadataDescribeService>;
  readonly transmogrifierService: InstanceType<typeof TransmogrifierService>;
};

export const makeOrgCatalogSObjects = ({
  state,
  metadataDescribeService,
  transmogrifierService
}: OrgCatalogSObjectsOptions) => {
  ...
  const refreshSObjects = Effect.fn('OrgCatalogSObjects.refreshSObjects')(function* (
    ...
    yield* state.ensureHydrated(orgId);
    yield* Effect.all(
      [metadataDescribeService.invalidateListSObjects(), metadataDescribeService.invalidateSObjectDescribes(apiNames)],
      { discard: true }
    );
