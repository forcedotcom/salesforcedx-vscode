...
type WorkspaceState = {
  readonly getWorkspaceTypes: (orgId: string) => Effect.Effect<ReadonlySet<string> | undefined>;
  readonly setWorkspaceTypes: (orgId: string, types: ReadonlySet<string>) => Effect.Effect<void>;
};

type OrgCatalogWorkspaceOptions = {
  readonly state: WorkspaceState;
  readonly projectService: InstanceType<typeof ProjectService>;
  readonly metadataRetrieveService: InstanceType<typeof MetadataRetrieveService>;
  readonly remoteDocumentUri: (orgId: string, reference: OrgMetadataComponentReference) => URI;
};

export const makeOrgCatalogWorkspace = ({
  state,
  projectService,
  metadataRetrieveService,
  remoteDocumentUri
}: OrgCatalogWorkspaceOptions) => {
  ...
  const resolveKnownOrgComponents = Effect.fn('OrgCatalogWorkspace.resolveKnownOrgComponents')(function* (
    ...
    const resolutions = references.map(reference => {
      ...
      return {
        reference,
        documentUri: workspaceUri ?? remoteDocumentUri(orgId, reference),
