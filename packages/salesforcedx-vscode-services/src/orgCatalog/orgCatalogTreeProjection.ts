...
import type { makeOrgCatalogSObjects } from './orgCatalogSObjects';
...
import { isOrgMetadataComponentReference, type OrgMetadataReference } from './orgMetadataReference';

type OrgCatalogTreeProjectionOptions = {
  readonly inventories: ReturnType<typeof makeOrgCatalogInventory>;
  readonly sobjects: ReturnType<typeof makeOrgCatalogSObjects>;
...
export const makeOrgCatalogTreeProjection = ({
  inventories,
  sobjects,
  workspace,
  metadataDescribeService,
  entryUri
}: OrgCatalogTreeProjectionOptions) => {
  ...
  const getChildren = Effect.fn('OrgCatalogTreeProjection.getChildren')(function* (
    ...
    if (!reference.xmlName) {
      const [metadataTypes, workspaceTypes] = yield* Effect.all(
        [metadataDescribeService.describe(), workspace.getWorkspaceMetadataTypes(orgId)],
