...
- Org metadata VFS (`src/orgVfs`) — a read-only `sf-org-data` virtual filesystem of org-derived metadata. `OrgMetadataCatalog` provides discovery, presence, and inventory; `OrgMetadataResolver` provides content and the filesystem projection. Consuming extensions use them to discover, badge, and open components without their own fetch or presence code
