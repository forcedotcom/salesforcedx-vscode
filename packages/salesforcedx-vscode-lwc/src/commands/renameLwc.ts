...
export const renameLwcCommand = Effect.fn('renameLwcCommand')(function* (
  ...
  const newName = normalizeComponentName(
    yield* promptForLwcName({ existingNames, initialValue: oldName, bundleFileNames }),
    'lwc'
  );

  const oldMainFile =
    bundleFileNames.find(
      f => bundleFilePattern(oldName, 'lwc').test(f) && /\.(ts|js)$/.test(f) && !f.includes('.test.')
    ) ?? `${oldName}.js`;
  const oldMainUri = Utils.joinPath(bundleUri, oldMainFile);

  const newBundleUri = yield* lightningComponentService.renameBundle({ bundleUri, oldName, newName, kind: 'lwc' });
