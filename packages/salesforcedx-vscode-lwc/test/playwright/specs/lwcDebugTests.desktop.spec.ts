...
/** Focus the Test Explorer, refresh discovery, and wait for the generated lwc1 suite. */
const openAndRefreshTestExplorer = async (page: Page): Promise<void> => {
  const lwc1 = page.getByRole('treeitem', { name: /lwc1/i, level: 1 });
  await executeCommandWithCommandPalette(page, 'Testing: Focus on Test Explorer View');
  await expect(lwc1).toBeVisible({ timeout: 30_000 });

  // Observe VS Code's Testing-sidebar progress lifecycle. The action label is a fallback for VS Code
  // versions that replace Refresh Tests with Cancel Test Refresh for the refreshHandler lifetime.
  const body = page.locator('body');
  await body.evaluate((element, testExplorerPanel) => {
    element.dataset.lwcTestRefresh = 'waiting';
    const observer = new MutationObserver(mutations => {
      for (const mutation of mutations) {
        if (!(mutation.target instanceof Element)) continue;

        const currentLabel = mutation.target.getAttribute('aria-label');
        const cancelActionStarted = currentLabel?.startsWith('Cancel Test Refresh') === true;
        const cancelActionFinished =
          mutation.oldValue?.startsWith('Cancel Test Refresh') === true && !cancelActionStarted;

        const sidebar = mutation.target.closest('[id="workbench.parts.sidebar"]');
        const isTestingProgress =
          mutation.target.classList.contains('monaco-progress-container') &&
          Boolean(sidebar?.querySelector(testExplorerPanel));
        const progressStarted = isTestingProgress && mutation.target.classList.contains('active');
        const progressFinished =
          isTestingProgress && mutation.oldValue?.split(/\s+/).includes('active') === true && !progressStarted;

        if (cancelActionStarted || progressStarted) {
          element.dataset.lwcTestRefresh = 'refreshing';
        }
        if (cancelActionFinished || progressFinished) {
          element.dataset.lwcTestRefresh = 'complete';
          observer.disconnect();
          return;
        }
      }
  ...
  await expect(body, 'Test Explorer refresh should complete').toHaveAttribute('data-lwc-test-refresh', 'complete', {
    timeout: 30_000
  });
  await expect(lwc1).toBeVisible({ timeout: 30_000 });
};
