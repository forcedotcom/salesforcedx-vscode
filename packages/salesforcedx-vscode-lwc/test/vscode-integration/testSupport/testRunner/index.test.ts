...
import { TestCaseInfo, TestInfoKind, TestType } from '../../../../src/testSupport/types';
...
describe('LWC Test Runner', () => {
  describe('Jest Execution Info Unit Tests', () => {
    let randomUUIDStub: SinonStub<[crypto.RandomUUIDOptions?], string>;
    let getTempFolderStub: SinonStub<[string, string], string>;
    beforeEach(() => {
      randomUUIDStub = stub(crypto, 'randomUUID');
      const mockUuid = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
      randomUUIDStub.returns(mockUuid);
    ...
    const salesforceProjectPath = path.join(root, 'project', 'mockSalesforceProject');
    ...
    it('Should get jest execution info for test case', () => {
      ...
      const jestExecutionInfo = new TestRunner(testExecutionInfo, TestRunType.RUN).getJestExecutionInfo(
        mockWorkspaceFolder as vscode.WorkspaceFolder
      );
