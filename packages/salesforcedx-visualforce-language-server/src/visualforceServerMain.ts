...
import { getVisualforceDataProvider } from './visualforceDataProvider';
...
connection.onInitialize(
  (params: InitializeParams): InitializeResult => {
    ...
    const visualforceDataProvider = getVisualforceDataProvider();
    providers.push(visualforceDataProvider);
    ...
    clientDynamicRegisterSupport = getClientCapability(
      ...
      if (latestTextDocument && latestTextDocument.version === version) {
        ...
        modes.forEach(mode => {
          if (
            ...
            pushAll(
              diagnostics,
              mode.doValidation(latestTextDocument, settings)
            );
          }
        });
