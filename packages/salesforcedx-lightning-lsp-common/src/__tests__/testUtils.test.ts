...
it('readAsTextDocument()', async () => {
  ...
  await fileSystemProvider.updateFileContent(
  ...
  // reads .js file
  let document = await readAsTextDocument(`${FORCE_APP_ROOT}/lwc/hello_world/hello_world.js`, fileSystemProvider);
  ...
  // reads .html file
  document = await readAsTextDocument(`${FORCE_APP_ROOT}/lwc/hello_world/hello_world.html`, fileSystemProvider);
  ...
  document = await readAsTextDocument(`${FORCE_APP_ROOT}/aura/helloWorldApp/helloWorldApp.app`, fileSystemProvider);
  expect(document.languageId).toBe('html');
  document = await readAsTextDocument(`${FORCE_APP_ROOT}/aura/wireLdsCmp/wireLdsCmp.cmp`, fileSystemProvider);
  expect(document.languageId).toBe('html');
});
