...
it('readAsTextDocument()', async () => {
    // reads .js file
    let document = await readAsTextDocument(`${FORCE_APP_ROOT}/lwc/hello_world/hello_world.js`);
    ...
    // reads .html file
    document = await readAsTextDocument(`${FORCE_APP_ROOT}/lwc/hello_world/hello_world.html`);
    ...
    document = await readAsTextDocument(`${FORCE_APP_ROOT}/aura/helloWorldApp/helloWorldApp.app`);
    expect(document.languageId).toBe('html');
    document = await readAsTextDocument(`${FORCE_APP_ROOT}/aura/wireLdsCmp/wireLdsCmp.cmp`);
    expect(document.languageId).toBe('html');
});
