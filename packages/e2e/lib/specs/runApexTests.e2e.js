...
describe('Run Apex Tests', async () => {
    ...
    (0, mocha_steps_1.step)('Run All Tests via Apex Class', async () => {
    ...
    (0, mocha_steps_1.step)('Run Single Test via Apex Class', async () => {
    ...
    (0, mocha_steps_1.step)('Run All Tests via Command Palette', async () => {
    ...
    (0, mocha_steps_1.step)('Run Single Class via Command Palette', async () => {
    ...
    (0, mocha_steps_1.step)('Run All tests via Test Sidebar', async () => {
        ...
        const apexTestsItems = await utilities.verifyTestItemsInSideBar(apexTestsSection, 'Refresh Tests', expectedItems, 6, 3);
        // Clear the Output view.
        await utilities.dismissAllNotifications();
        await utilities.clearOutputView(utilities.Duration.seconds(2));
    ...
    (0, mocha_steps_1.step)('Run All Tests on a Class via the Test Sidebar', async () => {
        utilities.log(`RunApexTests - Run All Tests on a Class via the Test Sidebar`);
        const workbench = utilities.getWorkbench();
        // Clear the Output view.
        await utilities.dismissAllNotifications();
        await utilities.clearOutputView(utilities.Duration.seconds(2));
        const outputPanelText = await utilities.runTestCaseFromSideBar(workbench, 'Apex Tests', 'ExampleApexClass2Test', 'Run Tests');
        const expectedTexts = [
            '=== Test Summary',
            'Outcome              Passed',
            'Tests Ran            1',
    ...
    (0, mocha_steps_1.step)('Run Single Test via the Test Sidebar', async () => {
        ...
        await utilities.dismissAllNotifications();
        await utilities.clearOutputView(utilities.Duration.seconds(2));
        const outputPanelText = await utilities.runTestCaseFromSideBar(workbench, 'Apex Tests', 'validateSayHello', 'Run Single Test');
        const expectedTexts = [
            '=== Test Summary',
            'Outcome              Passed',
            'Tests Ran            1',
    ...
    (0, mocha_steps_1.step)('Run a test that fails and fix it', async () => {
        ...
        try {
            successPushNotification2WasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Push Source to Default Org and Ignore Conflicts successfully ran', utilities.Duration.TEN_MINUTES);
            (0, chai_1.expect)(successPushNotification2WasFound).to.equal(true);
        }
        catch (error) {
            await utilities.getWorkbench().openNotificationsCenter();
            successPushNotification2WasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Push Source to Default Org and Ignore Conflicts successfully ran', utilities.Duration.ONE_MINUTE);
            (0, chai_1.expect)(successPushNotification2WasFound).to.equal(true);
        }
        // Clear the Output view.
        await utilities.dismissAllNotifications();
        ...
        await prompt.confirm();
        // Look for the success notification that appears which says, "SFDX: Run Apex Tests successfully ran".
        const successNotification2WasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Run Apex Tests successfully ran', utilities.Duration.TEN_MINUTES);
    ...
    (0, mocha_steps_1.step)('Create Apex Test Suite', async () => {
        ...
        const checkbox = await prompt.findElement(vscode_extension_tester_1.By.css('input.quick-input-list-checkbox'));
        ...
        const successNotificationWasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Build Apex Test Suite successfully ran', utilities.Duration.TEN_MINUTES);
        (0, chai_1.expect)(successNotificationWasFound).to.equal(true);
    });
    (0, mocha_steps_1.step)('Add test to Apex Test Suite', async () => {
    ...
    (0, mocha_steps_1.step)('Run Apex Test Suite', async () => {
...
//# sourceMappingURL=runApexTests.e2e.js.map