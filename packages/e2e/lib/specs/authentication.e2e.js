...
describe('Authentication', async () => {
    ...
    (0, mocha_steps_1.step)('Run SFDX: Authorize a Dev Hub', async () => {
    ...
    (0, mocha_steps_1.step)('Run SFDX: Set a Default Org', async () => {
        ...
        const orgPickerOptions = await workbench.findElements(vscode_extension_tester_1.By.css('div.monaco-list#quickInput_list > div.monaco-scrollable-element > div.monaco-list-rows > div.monaco-list-row'));
        ...
        const expectedOutputWasFound = await utilities.attemptToFindOutputPanelText('Salesforce CLI', `target-org  ${devHubAliasName}  true`, 5);
    ...
    (0, mocha_steps_1.step)('Run SFDX: Create a Default Scratch Org', async () => {
        const prompt = await utilities.executeQuickPick('SFDX: Create a Default Scratch Org...', utilities.Duration.seconds(1));
        ...
        // Press Enter/Return.
        await prompt.confirm();
        const successNotificationWasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Create a Default Scratch Org... successfully ran', utilities.Duration.TEN_MINUTES);
        if (successNotificationWasFound !== true) {
            const failureNotificationWasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Create a Default Scratch Org... failed to run', utilities.Duration.TEN_MINUTES);
    ...
    (0, mocha_steps_1.step)('Run SFDX: Set the Scratch Org As the Default Org', async () => {
