...
describe('Push and Pull', async () => {
    ...
    (0, mocha_steps_1.step)('Set up the testing environment', async () => {
        ...
        testSetup1 = await testSetup_1.TestSetup.setUp(testReqConfig);
        projectName = testSetup1.tempProjectName;
    });
    (0, mocha_steps_1.step)('SFDX: View All Changes (Local and in Default Org)', async () => {
        utilities.log('Push And Pull - SFDX: View All Changes (Local and in Default Org)');
        await utilities.executeQuickPick('SFDX: View All Changes (Local and in Default Org)', utilities.Duration.seconds(5));
        // Check the output.
    ...
    (0, mocha_steps_1.step)('SFDX: View Local Changes', async () => {
        ...
        // Check the output.
        const outputPanelText = await utilities.attemptToFindOutputPanelText('Salesforce CLI', 'Source Status', 10);
        (0, chai_1.expect)(outputPanelText).to.contain(`Local Add  ExampleApexClass1  ApexClass  ${path_1.default.join('force-app', 'main', 'default', 'classes', 'ExampleApexClass1.cls')}`);
        (0, chai_1.expect)(outputPanelText).to.contain(`Local Add  ExampleApexClass1  ApexClass  ${path_1.default.join('force-app', 'main', 'default', 'classes', 'ExampleApexClass1.cls-meta.xml')}`);
    });
    (0, mocha_steps_1.step)('Push the Apex class', async () => {
    ...
    (0, mocha_steps_1.step)('Push again (with no changes)', async () => {
    ...
    (0, mocha_steps_1.step)('Modify the file and push the changes', async () => {
    ...
    (0, mocha_steps_1.step)('Pull the Apex class', async () => {
    ...
    (0, mocha_steps_1.step)("Modify the file (but don't save), then pull", async () => {
    ...
    (0, mocha_steps_1.step)('Save the modified file, then pull', async () => {
    ...
    (0, mocha_steps_1.step)('SFDX: View Changes in Default Org', async () => {
        ...
        testSetup2 = await testSetup_1.TestSetup.setUp(testReqConfig2);
    ...
    (0, mocha_steps_1.xstep)('Create an additional system admin user', async () => {
        ...
        const systemAdminUserDefPath = path_1.default.join(testSetup2.projectFolderPath, 'config', 'system-admin-user-def.json');
    ...
    (0, mocha_steps_1.xstep)('Set the 2nd user as the default user', async () => {
     ...
     */
    const verifyPushAndPullOutputText = async (operation, fromTo, type) => {
        const successNotificationWasFound = await utilities.notificationIsPresentWithTimeout(`SFDX: ${operation} Source ${fromTo} Default Org successfully ran`, utilities.Duration.TEN_MINUTES);
        (0, chai_1.expect)(successNotificationWasFound).to.equal(true);
        // Check the output.
        const outputPanelText = await utilities.attemptToFindOutputPanelText('Salesforce CLI', `=== ${operation}ed Source`, 10);
        (0, chai_1.expect)(outputPanelText).to.not.be.undefined;
        if (type) {
            if (operation === 'Push') {
                (0, chai_1.expect)(outputPanelText).to.contain(`${type}  ExampleApexClass1  ApexClass`);
            }
            else {
                (0, chai_1.expect)(outputPanelText).to.contain(`${type}  Admin`);
            }
        }
        else {
...
async function verifyPushSuccess(wait = utilities.Duration.TEN_MINUTES) {
    const successNotificationWasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Push Source to Default Org successfully ran', wait);
    (0, chai_1.expect)(successNotificationWasFound).to.equal(true);
}
async function verifyPullSuccess(wait = utilities.Duration.TEN_MINUTES) {
    const successNotificationWasFound = await utilities.notificationIsPresentWithTimeout('SFDX: Pull Source from Default Org successfully ran', wait);
    (0, chai_1.expect)(successNotificationWasFound).to.equal(true);
}
//# sourceMappingURL=pushAndPull.e2e.js.map