...
const findTsFiles = (packageRoot: string): string[] => {
  const result: string[] = [];
  const searchDirs = ['src', 'test'].map(d => path.join(packageRoot, d)).filter(d => fs.existsSync(d));
...
const loadPackageNlsKeys = (packageRoot: string): Set<string> => {
  try {
    const content = fs.readFileSync(path.join(packageRoot, 'package.nls.json'), 'utf8');
...
const collectReferenceCountsFromSource = (source: string, knownKeys: Set<string>): Map<string, number> => {
  ...
  const visitor = (node: TSESTree.Node): void => {
    switch (node.type) {
      ...
      case AST_NODE_TYPES.Literal: {
        if (typeof node.value === 'string' && knownKeys.has(node.value)) {
...
export const noUnusedI18nMessages = RuleCreator.withoutDocs({
  meta: {
    ...
    schema: [
      {
        type: 'object',
        description: 'Options for configuring unused i18n message detection.',
        properties: {
          allowList: {
            description: 'Keys that should never be reported as unused.',
            type: 'array',
            items: { type: 'string' }
          },
          dynamicKeyPatterns: {
            description: 'Regex patterns for keys used dynamically at runtime.',
            type: 'array',
            items: { type: 'string' }
          }
        },
  ...
  create: (context, [opts = DEFAULT_OPTIONS]) => {
    ...
    for (const file of findTsFiles(packageRoot)) {
      ...
      try {
        ...
        const fileCounts = collectReferenceCountsFromSource(source, knownKeys);
    ...
    if (normalizedPath.endsWith('/modules/querybuilder/messages/i18n.ts')) {
      for (const file of findSoqlBuilderUiHtmlFiles(packageRoot)) {
        try {
          const source = fs.readFileSync(file, 'utf8');
          const fileCounts = collectQueryBuilderI18nKeyRefsFromHtml(source, knownKeys);
          for (const [k, c] of fileCounts) {
            refCounts.set(k, (refCounts.get(k) ?? 0) + c);
          }
        } catch {
    ...
    return {
      ...
      Property: (node: TSESTree.Property) => {
        ...
        if ((refCounts.get(key) ?? 0) > 0 || allowList.has(key) || isKeyExcludedByPattern(key, dynamicPatterns)) {
          return;
        }
